...
### v2.1.1 Release Notes (Consolidated)
...
4.  **Anti-Rug Protection:** Real-time mempool log analysis triggers emergency sells if rugpull indicators are detected.
...
-   **`backend/`**: Flask API & Async Services.
    -   `src/services/mempool_monitor.py`: Real-time Solana event listener.
    ...
    -   `src/utils/db.py`: SQLite persistence layer.
