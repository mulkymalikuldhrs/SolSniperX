...
def get_tokens():
    ...
    try:
        tokens = run_async(data_fetcher_service.get_all_tokens())
        return success_response(data=tokens, count=len(tokens))
    except Exception as e:
        logger.error(f"Error fetching tokens: {str(e)}")
        return error_response('Failed to fetch tokens', details=str(e))
...
def get_token_details(token_address):
    ...
    data_fetcher_service = current_app.services['data_fetcher']
    try:
        token = run_async(data_fetcher_service.get_token_by_address(token_address))
        ...
        return error_response('Failed to fetch token details', details=str(e))

@tokens_bp.route('/<token_address>/history', methods=['GET'])
def get_token_history(token_address):
    ...
    try:
        ...
        history = run_async(data_fetcher_service.get_historical_prices(token_address, interval, limit))
        ...
        return error_response('Failed to fetch token history', details=str(e))