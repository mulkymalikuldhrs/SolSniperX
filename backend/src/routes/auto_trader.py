...
@auto_trader_bp.route('/status', methods=['GET'])
def get_status():
    """Gets the automated trading bot status."""
    auto_trader_service = current_app.services['auto_trader']
    return success_response(data={'enabled': auto_trader_service.trading_enabled})
...
def start_auto_trader():
    ...
    try:
        ...
        return error_response('Failed to start automated trading', details=str(e))
...
def stop_auto_trader():
    ...
    try:
        ...
        return error_response('Failed to stop automated trading', details=str(e))