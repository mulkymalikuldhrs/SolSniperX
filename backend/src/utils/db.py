...
def init_db():
    ...
    # Add token_symbol column to trades if it's missing (migration)
    try:
        cursor.execute('ALTER TABLE trades ADD COLUMN token_symbol TEXT')
...
def record_trade(trade_data):
    ...
    try:
        ...
        ''', (
            ...
            trade_data.get('token_symbol') or trade_data.get('token_address')[:8],
            trade_data.get('type'),
        ...
        cursor.execute('''
        UPDATE trades SET status = ? WHERE transaction_id = ?
        ''', (trade_data.get('status'), trade_data.get('transaction_id')))
        conn.commit()
