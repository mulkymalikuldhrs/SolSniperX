        ...
        while True:
            try:
                ...
                try:
                    ...
                    logger.error(f"Watchdog error checking Mempool Monitor: {e}")
                ...
                try:
                    if auto_trader_service.trading_enabled:
                        if auto_trader_service.trade_loop_task is None or auto_trader_service.trade_loop_task.done():
                            ...
                            # Resetting trading_enabled to False allows start_trading to re-initiate the loop
                            auto_trader_service.trading_enabled = False
                            auto_trader_service.start_trading()
                except Exception as e:
                    logger.error(f"Watchdog error checking Auto Trader: {e}")

            except Exception as e:
                logger.error(f"General error in service watchdog: {e}")
