...
import json
import os
from utils.db import save_position, remove_position, get_active_positions
...
class AutoTraderService:
    ...
    async def _scan_and_buy(self):
        ...
        try:
            ...
            if not all_tokens:
                logger.warning("AutoTrader: No tokens returned from data fetcher.")
                return
            
            for token in all_tokens:
                try:
                    liquidity = token.get('liquidity', 0)
                    if (liquidity < self.config.get("min_liquidity", 0) or
                        liquidity > self.config.get("max_liquidity", float('inf')) or
                        token.get('age_hours', 0) > self.config.get("max_age_hours", 24)):
                        continue
                    ...
                    logger.error(f"AutoTrader: Error processing token {token.get('address')}: {token_e}")

        except Exception as e:
            logger.exception(f"AutoTrader: Critical error during scan and buy: {e}")

    async def _monitor_and_sell(self):
        tokens_to_remove = []
        for token_address, details in list(self.owned_tokens.items()):
            try:
                # Real balance check
                try:
                    current_balance = await self.wallet_service.get_token_balance(token_address)
                except Exception as balance_e:
                    logger.error(f"AutoTrader: Error checking balance for {token_address}: {balance_e}")
                    continue
    ...
    async def handle_new_token(self, token_data: Dict):
        ...
        # If liquidity is known, check against bounds
        if liquidity > 0:
             if liquidity < self.config.get('min_liquidity', 0) or \
                liquidity > self.config.get('max_liquidity', float('inf')):
                 return

        if age_hours > self.config.get('max_age_hours', 24):
            return

        logger.info(f"AutoTrader: New token {token_address} detected via mempool, analyzing...")
    ...
    async def _analyze_and_buy(self, token: Dict):
        ...
        if analysis and analysis.get('recommendation') == "Buy" and \
           analysis.get('probability_score', 0) >= self.config.get("min_ai_probability_score", 75) and \
           analysis.get('risk_assessment') != "High":

            logger.info(f"AutoTrader: AI recommends BUY for {token.get('symbol', token_address)} (Score: {analysis.get('probability_score')}, Risk: {analysis.get('risk_assessment')}).")
            ...
            buy_result = await self.trading_service.execute_buy_order(
                token_address=token_address,
                amount_sol=self.config.get("buy_amount_sol", 0.05),
                slippage=self.config.get("slippage", 1.5),
