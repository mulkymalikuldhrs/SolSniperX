...
class AutoTraderService:
    ...
    def __init__(self, socketio=None, data_fetcher_service=None, ai_analysis_service=None, trading_service=None, wallet_service=None):
        ...
        self.owned_tokens: Dict[str, Dict] = {p['token_address']: p for p in db_positions}
    ...
    def start_trading(self, loop=None):
        if not self.trading_enabled:
            ...
            if not target_loop:
                try:
                    import main
                    target_loop = getattr(main, 'background_loop', None)
                except ImportError:
                    pass

            if target_loop and target_loop.is_running():
                self.trade_loop_task = asyncio.run_coroutine_threadsafe(self._trade_loop(), target_loop)
            else:
                ...
                if analysis and analysis['recommendation'] == "Buy" and \
                   ...
                   analysis['risk_assessment'] != "High":
                    ...
                    if buy_result.get("success"):
                        # Verification of actual balance received
                        await asyncio.sleep(5)
                        token_balance = await self.wallet_service.get_token_balance(token['address'])
                        ...
                        if token_balance > 0:
                            ...
                            self.owned_tokens[token['address']] = position_data
