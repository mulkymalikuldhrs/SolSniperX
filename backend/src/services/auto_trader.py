...
class AutoTraderService:
    ...
    def _load_config(self) -> Dict:
        ...
        return {
            ...
            "profit_target_x": 2.0, # Frontend compatibility
    ...
    def start_trading(self):
        # Allow starting if either not enabled OR if task is done (meaning it crashed or finished)
        is_task_running = self.trade_loop_task is not None and not self.trade_loop_task.done()

        if not self.trading_enabled or not is_task_running:
            ...
            logger.info("AutoTrader: Automated trading started/restarted.")
            if self.socketio:
