    ...
    async def _monitor_and_sell(self):
        ...
        for token_address, details in self.owned_tokens.items():
            try:
                ...
                # 1. Check Multiple Take-Profit Tiers & profit_target_x Compatibility
                tp_tiers = self.config.get("take_profit_tiers", []).copy()

                # Compatibility: Add profit_target_x as a 100% sell tier if not present
                profit_target_x = self.config.get("profit_target_x")
                if profit_target_x and not any(t['target_x'] == profit_target_x for t in tp_tiers):
                    tp_tiers.append({"target_x": profit_target_x, "sell_percentage": 1.0})

                # Sort tiers by target_x to ensure logical execution
