...
class AutoTraderService:
    ...
    def _ensure_profit_target_compatibility(self):
        ...
        Maintains only one 100% sell tier to avoid redundant sell executions.
        """
        ...
        # Check if we already have this exact target with 100% sell
        has_exact = any(t.get('target_x') == profit_target_x and t.get('sell_percentage') == 1.0 for t in tp_tiers)

        if not has_exact:
            logger.info(f"AutoTrader: Syncing profit_target_x {profit_target_x} to take_profit_tiers.")
            # Remove any existing 100% sell tiers to prevent accumulation of old targets
            tp_tiers = [t for t in tp_tiers if t.get('sell_percentage', 0) < 1.0]
            # Add the new definitive final target
            tp_tiers.append({"target_x": profit_target_x, "sell_percentage": 1.0})
            # Keep tiers sorted by target price
