...
class AutoTraderService:
    ...
    async def post_init(self):
        ...
        # Ensure profit_target_x compatibility
    ...
    def _ensure_profit_target_compatibility(self):
        ...
        if "profit_target_x" in self.config:
            ...
            if not any(t.get('target_x') == target_x for t in tp_tiers):
                logger.info(f"AutoTrader v3.3.0: Normalizing config - Appending implicit profit_target_x tier: {target_x}x")
    ...
    async def _monitor_and_sell(self):
        ...
        for token_address, details in self.owned_tokens.items():
            try:
                ...
                # 1. Check Multiple Take-Profit Tiers (or single profit_target_x)
                tp_tiers = list(self.config.get("take_profit_tiers", []))
                profit_target_x = self.config.get("profit_target_x")

                # Ensure profit_target_x is handled if provided by frontend
                if profit_target_x and not any(t.get('target_x') == profit_target_x for t in tp_tiers):
