        ...
        try:
            ...
            for instruction in transaction_data.transaction.message.instructions:
                ...
                if program_id == str(TOKEN_PROGRAM_ID):
                    ...
                    # For Pump.fun 'Create' instruction, the mint is typically the first account
                    if hasattr(instruction, 'accounts') and len(instruction.accounts) > 0:
                        mint_index = instruction.accounts[0]
                        # account_keys is a list of Pubkeys or parsed objects
                        account_keys = transaction_data.transaction.message.account_keys
                        mint_pubkey = account_keys[mint_index]
                        if hasattr(mint_pubkey, 'pubkey'):
                            await self._emit_new_token(str(mint_pubkey.pubkey))
                        else:
                            await self._emit_new_token(str(mint_pubkey))
                        return
