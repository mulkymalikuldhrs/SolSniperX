...
class TradingService:
    ...
    async def _get_swap_instructions(self, input_mint: str, output_mint: str, amount: int, slippage_bps: int) -> Optional[Dict]:
        ...
        try:
            ...
            response = await self.http_client.get(f"{JUPITER_API_BASE_URL}/quote", params=params, timeout=10.0)
            if response.status_code != 200:
                logger.error(f"Jupiter quote API returned {response.status_code}: {response.text}")
                return None

            quote_data = response.json()
            if not quote_data:
                logger.error("Empty quote data from Jupiter.")
                return None
            ...
            response = await self.http_client.post(f"{JUPITER_API_BASE_URL}/swap", json=swap_payload, timeout=15.0)
            if response.status_code != 200:
                logger.error(f"Jupiter swap API returned {response.status_code}: {response.text}")
                return None
            ...
            logger.error(f"Jupiter API returned HTTP error: {e}")
            return None
        except httpx.RequestError as e:
            logger.error(f"Error connecting to Jupiter API: {e}")
            return None
        except Exception as e:
    ...
    async def _confirm_transaction(self, signature: Signature, timeout: int = 60, interval: int = 2) -> bool:
        """
        Polls for transaction confirmation. Returns True if confirmed, False otherwise.
        """
            ...
            try:
                ...
                if response.value and response.value[0]:
                    ...
                    if status.err:
                        ...
                        return False
                    if status.confirmations is not None or status.confirmation_status in ['confirmed', 'finalized']:
                        return True
            except Exception as e:
